...
public:
	...
	float		DrawLength ( const char *szText) const;
	size_t		GetCharPos( const char *text, float x) const;
...
public:
	...
	void	D3DTexCoord (float uv[2]);
	void	D3DTexCoord2f ( float u, float v );

	HRESULT	D3DVertex(float xy[2]);
	...
	void	DrawTexture(float x, float y, float w, float h, LPDIRECT3DTEXTURE9 t, D3DCOLOR color = 0xFFFFFFFF);
private:
