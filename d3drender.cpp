...
void CD3DRender::D3DTexCoord(float uv[2])
{
	m_tu = uv[0];
	m_tv = uv[1];
}
...
HRESULT	CD3DRender::D3DVertex(float xy[2])
{
	if (m_canRender && m_pVertex && ++m_curVertex < m_maxVertex)
		*m_pVertex++ = Init2DVertex(xy[0], xy[1], m_color, m_tu, m_tv);
	else
		return E_FAIL;
...
void CD3DRender::DrawTexture(float x, float y, float w, float h, LPDIRECT3DTEXTURE9 t, D3DCOLOR color)
{
	D3DBindTexture(t);
